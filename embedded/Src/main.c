...
static void handle_user_button(app_context_t *ctx)
{
    ...
    /* Detect button release */
        ...
        if (duration >= BUTTON_LONG_PRESS_DURATION_MS) {  // Long press
            printf("Long press detected - resetting face data...\r\n");
            embeddings_bank_reset();
            embeddings_erase_from_flash();  // Also erase from flash
            printf("✓ Face data cleared from memory and flash\r\n");
        } 
        else if (ctx->embedding_valid) {  // Short press
            int count = embeddings_bank_add(ctx->current_embedding);
            if (count > 0) {
                printf("Added embedding %d/%d\r\n", count, EMBEDDING_BANK_SIZE);
