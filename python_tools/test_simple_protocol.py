...
def main():
    ...
    # Find ST-Link VCP automatically
    stlink_port = None
    for port in ports:
        if 'STLINK' in port.description or 'ST-Link' in port.description:
            stlink_port = port.device
            logger.info(f"Found ST-Link VCP: {stlink_port}")
            break
    
    if not stlink_port:
        # Look for ttyACM devices (typical for ST-Link VCP)
        acm_ports = [p for p in ports if 'ttyACM' in p.device]
        if acm_ports:
            stlink_port = acm_ports[0].device
            logger.info(f"Using ACM port (likely ST-Link): {stlink_port}")
        else:
            logger.error("No ST-Link VCP found!")
            logger.error("Make sure STM32N6570 Discovery is connected via USB to ST-Link port")
            logger.info("Expected device: /dev/ttyACM0 with ST-Link description")
            return
    
    port_name = stlink_port
    baud_rate = 7372800
    
    logger.info(f"Testing simple protocol on {port_name} at {baud_rate} baud...")
    logger.info("Using ST-Link Virtual COM Port (VCP) - PE5/PE6 → COM1")
    ...
    try:
        ...
        logger.info("1. Make sure STM32N6570 Discovery is connected via USB")
        logger.info("2. Check that ST-Link drivers are installed")
        logger.info("3. Verify STM32N6 firmware is built and flashed")
        logger.info("4. Confirm COM1/UART is configured correctly in STM32 code")
        logger.info("5. Check if you're in the 'dialout' group: groups $USER")
